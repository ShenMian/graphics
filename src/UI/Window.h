...
public:
	enum Flag
	{
		...
		NoScrollWithMouse = 1 << 4, // 禁止用户使用鼠标滚轮垂直滚动. On child window, mouse wheel will be forwarded to
		                            // the parent unless NoScrollbar is also set.
		NoCollapse =
		    1 << 5, // 禁止用户通过双击折叠窗口. Also referred to as Window Menu Button (e.g. within a docking node).
		...
		HorizontalScrollbar =
		    1 << 11, // 允许水平滚动条显示(默认关). You may use SetNextWindowContentSize(ImVec2(width,0.0f)); prior to
		...
		AlwaysUseWindowPadding    = 1 << 16, // Ensure child windows without border uses style.WindowPadding (ignored by
		...
		UnsavedDocument =
		    1 << 20, // 在标题旁显示一个点. When used in a tab/docking context, tab is selected when clicking the X +
		             // closure is not assumed (will wait for user to stop submitting the tab). Otherwise closure is
