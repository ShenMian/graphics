...
void compressIndices(std::vector<uint8_t>& buffer, const void* indices, size_t indexCount, size_t vertexCount)
{
	...
	buffer.resize(meshopt_encodeIndexBuffer(buffer.data(), buffer.size(),
	                                        reinterpret_cast<const unsigned int*>(indices), indexCount));
}
...
Mesh::Mesh(std::string_view name, std::vector<Vertex>& vertices, std::vector<unsigned int>& indices,
           const Material* mat)
